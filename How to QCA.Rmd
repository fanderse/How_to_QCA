...
In this section we will deal with a categorical variable, GOV_TYPE. Our aim is to calibrate a set called "Weak Government". Again, before we go into the actual calibration, let's have a look at the data.
...
Keep in mind that *any* value which satisfies $0 <= value <= 1$ could be applied, e.g. 0, 0.1, 0.45, 0.78901, and 1. In practice, I think values with more than two decimals spaces should be avoided (more on this below) and a certain symmetry in values should be adhered to. This means a set which looks like ${0, 0.4, 0.6, 1}$ should be preferred to one which looks like ${0, 0.1, 0.2, 1}$, unless there is very good reason not to. As always, it is the interplay between case knowledge and empirical data which dictates this choice. 
